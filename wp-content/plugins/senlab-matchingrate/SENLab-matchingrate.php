...
function senlab_matchingrate_enqueue_scripts(){
	wp_enqueue_script('senlab-matchingrate',plugins_url('senlab-matchingrate/assets/js/matchingrate.js',dirname(__FILE__)),['jquery'], null);
	$localized_data = [
...
add_action('wp_ajax_senlab_matching_role_change','change_student_role');
function change_student_role(){
	$id = $_POST['id'];
	wp_publish_post($id);
