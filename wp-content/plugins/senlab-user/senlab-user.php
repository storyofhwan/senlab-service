...
function sen_create_new_talent($user_id,$submitted){
	...
	if(user_can($user_id,'student')){
		if(isset($submitted['nickname'])) $postarr['post_title'] = $submitted['nickname'];
		...
		if(isset($post_id)){
			...
			update_post_meta($post_id, '_edu', false);
			update_post_meta($post_id, '_exp', false);
			update_post_meta($post_id, '_pub', false);
			update_post_meta($post_id, '_survey', false);
		}
	}
}
